...
public class ScriptEntityCustomRepoImpl implements ScriptEntityCustomRepo{
    @PersistenceContext
    ...
    public List<ScriptEntity> getScriptEntityByLimit(int pageNo, int perPage) {
        try {
            ...
            result = ((Long)this.entityManager
                    .createQuery("SELECT count(*) FROM ScriptEntity a ")

                    .getSingleResult()).longValue();
            TypedQuery typedQuery = this.entityManager.createQuery("select a from  ScriptEntity a ", ScriptEntity.class);
            System.out.println("Count  " + result);
            int first = (pageNo - 1) * perPage;
            typedQuery.setFirstResult(first);
            typedQuery.setMaxResults(perPage);
            List<ScriptEntity> list = typedQuery.getResultList();
            return list;
        } finally {
    ...
    public List<ScriptEntity> getAllScriptEntityByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
        ...
        Query q = null;
        String queryStr = "from ScriptEntity a where  ";
        int i = 0;
        ...
        TypedQuery typedQuery = this.entityManager.createQuery(queryStr, ScriptEntity.class);
        int j = 0;
        for (ColumnSearch columnSearch : groupSearchDTO.getColumns()) {
            if (columnSearch.getValue() != "" || !columnSearch.getValue().equalsIgnoreCase("")) {
