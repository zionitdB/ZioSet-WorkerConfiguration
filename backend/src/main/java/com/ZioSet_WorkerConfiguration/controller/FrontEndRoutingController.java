...
public class FrontEndRoutingController {
    ...
    public String index(HttpServletResponse response) {
        ...
        response.setHeader("Content-Security-Policy", "default-src 'self'; connect-src 'self' http://localhost:3000 http://localhost:5173 http://localhost:8085 http://4.213.97.72:8085 http://4.213.97.72:8084 https://zensar-agent.zioset.com https://zensar.zioset.com; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https: http:; frame-src 'self'; frame-ancestors 'none'; font-src 'self' https://fonts.gstatic.com; media-src 'self'; object-src 'none'; manifest-src 'self'; worker-src 'self'; form-action 'self';");
