...
const STEPS = [
  { id: 0, title: "Basic Information", icon: FileText, desc: "Name and description" },
  { id: 1, title: "Configuration", icon: Terminal, desc: "Script and Dependencies" },
    { id: 2, title: "Requirements", icon: Settings2, desc: "Platform & Parameters" },
...
const initialFormState = {
 ...
 const [form, setForm] = useState<any>(initialFormState);
  ...
  const onFileUpload = async (e: any, target: "main" | "dependency") => {
    ...
    if (target === "main") {
      ...
      if (form.allowedExtensions.length > 0) {
        const isValid = form.allowedExtensions.some((ext: string) => file.name.endsWith(ext));
        if (!isValid) return alert(`Allowed extensions: ${form.allowedExtensions.join(", ")}`);
      }
      const res = await uploadMutation.mutateAsync(file);
      setForm((prev: any) => ({ ...prev, scriptFile: file, scriptFileId: res.id }));
    } else {
  ...
  const addParameter = () => {
    update("parameters", [...form.parameters, { paramName: "", paramType: "TEXT", required: true, defaultValue: "" }]);
  };
...
const isStepValid = () => {
  switch (step) {
    ...
    case 1:
      ...
      if (form.scriptCategory === "TEXT" && !form.command.trim()) return false;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-2 mx-auto">
        <div className="mb-4">
          <h1 className="text-2xl font-bold tracking-tight ">Create Script Template</h1>
        </div>
<Separator></Separator>
  {!isSuccess  ? (  
        ...
        <div className="grid mt-4 grid-cols-[300px_1fr] gap-8">
            ...
            {STEPS.map((s, i) => (
              <button
                ...
                className={`w-full text-left flex items-center gap-4 p-4 rounded-lg transition-all border ${
                  step === i ? "bg-muted border-primary/20 shadow-sm ring-1 ring-primary/10" : "border-transparent text-slate-500"
                }`}
              >
                <div className={`h-8 w-8 flex items-center justify-center rounded-full shrink-0 ${step >= i ? "bg-primary text-white" : "bg-slate-200"}`}>
                  {step > i ? <CheckCircle className="h-5 w-5" /> : <s.icon className="h-4 w-4" />}
                </div>
                <div className="overflow-hidden">
                  <p className={`text-sm font-bold truncate ${step === i ? "text-primary" : ""}`}>{s.title}</p>
                  <p className="text-[11px] opacity-70 font-medium truncate">{s.desc}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          <Card className="  overflow-hidden">
            <div className="px-6 py-4 bg-muted border-b flex justify-between items-center">
              <span className="text-xs font-bold uppercase text-slate-500 tracking-widest">{STEPS[step].title}</span>
              <span className="text-xs font-bold text-primary">Step {step + 1} / 4</span>
            </div>
              ...
              {step === 0 && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="grid gap-2">
                    <Label className="text-xs font-bold uppercase text-slate-500 tracking-widest">Template Name</Label>
<Input
  value={form.templetName}
  onChange={(e) => update("templetName", e.target.value)}
  placeholder="Enter template name..."
  className={!form.templetName && step === 0 ? "border-red-500" : ""}
/>


                  </div>
                  <div className="grid gap-2">
                    <Label className="text-xs font-bold uppercase text-slate-500 tracking-widest">Description</Label>
                    <Textarea value={form.templetDescription} onChange={(e) => update("templetDescription", e.target.value)} rows={5} placeholder="What does this template do?" />
                  </div>
                </div>
              )}
 ...
 <div className="space-y-2">
                  ...
                  <div className="flex gap-4 items-end">
                    <div className="flex-1 space-y-2">
                      <Label className="text-xs font-bold uppercase text-slate-500">Script Type</Label>
                      <Select value={form.scriptType} onValueChange={handleScriptTypeChange}>
                        <SelectTrigger className="w-full"><SelectValue placeholder="Select type" /></SelectTrigger>
                        <SelectContent>
                          {scriptTypes?.map((t: any) => (
                            <SelectItem key={t.enumName} value={t.enumName}>{t.displayName}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>
 ...
 {form.scriptCategory === "TEXT" ? (
  ...
  <p className="text-[10px] text-foreground italic">
    Wrap your variable name in <span className="font-mono text-foreground bg-primary/50 px-1 rounded">${'{variableName}'}</span> to reference it. <br />
    For example, if your variable is <span className="font-bold text-purple-600">applicationName</span>, use <span className="font-mono text-foreground bg-primary/50 px-1 rounded">${'{applicationName}'}</span> in your script.
  </p>
</div>
                    ) : null}
                    
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase text-slate-500">Script Content</Label>
                    {form.scriptCategory === "FILE" ? (
                      <div className="border-2 border-dashed rounded-xl p-8 text-center hover:bg-primary/5 cursor-pointer relative transition-all group">
                        <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => onFileUpload(e, "main")} accept={form.allowedExtensions.join(",")} />
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Allowed: {form.allowedExtensions.join(", ")}</p>
                        {form.scriptFile && <Badge className="mt-4" variant="secondary">{form.scriptFile.name}</Badge>}
                      </div>
                    ) : (
...
<div className="space-y-2">
  <div className="relative min-h-52 font-mono text-sm border rounded-md overflow-hidden bg-slate-950">
    ...
    <div
      ...
      dangerouslySetInnerHTML={{
        __html: form.command
          ...
          .replace(
            /\$\{(.*?)\}/g,
            '<span class="bg-primary/40 rounded px-1 border border-primary/50">$&</span>'
          )
      }}
    />
    ...
    />
                      ...
                      // <Textarea className="font-mono bg-slate-900 text-green-400 " value={form.command} onChange={(e) => update("command", e.target.value)} placeholder="# Enter code here..." />
                    )}
                  </div>
                    ...
                    <div className="border-2 border-dashed rounded-xl p-8 text-center hover:bg-secondary/50 cursor-pointer relative transition-all group">
                      <input type="file" multiple className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => onFileUpload(e, "dependency")} />
                      <Package className="mx-auto h-12 w-12 text-muted-foreground group-hover:text-primary" />
                      <p className="mt-2 text-lg font-semibold">Drop dependency files here</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {form.dependencyFiles.map((f: File, i: number) => (
                        <Badge key={i} variant="secondary" className="pl-3 pr-1 py-1 gap-2 ">
                          {f.name}
                          <Button variant="ghost" size="icon" className="h-5 w-5 rounded-full" onClick={() => removeDependency(i)}><Trash2 className="h-3 w-3" /></Button>
                        </Badge>
                      ))}
                    </div>
                  </div>
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {platformList?.map((p: any) => (
          <Button
            key={p.enumName}
            variant={form.targetPlatforms.includes(p.enumName) ? "default" : "outline"}
            size="sm"
            className="rounded-full transition-all"
            onClick={() => {
              const exists = form.targetPlatforms.includes(p.enumName);
              update(
                "targetPlatforms",
                exists
                  ? form.targetPlatforms.filter((x: any) => x !== p.enumName)
...
<div className="mt-4 p-4 border border-primary/20 bg-primary/5 rounded-xl">
  <div className="flex items-start gap-3">
    ...
    <div>
      <h4 className="text-sm font-bold text-primary">Smart Variable Sync</h4>
      <p className="text-[11px] text-slate-600 leading-relaxed mt-1">
        We detected <Badge variant="secondary" className="px-1 h-4 text-[10px]">{form.parameters.length}</Badge> variables in your script. 
        Each <code className="bg-primary/20 px-1 rounded text-primary">${"{name}"}</code> you type in Step 1 automatically appears here as a required parameter.
      </p>
    </div>
  </div>
</div>
        ...
        <div className="space-y-2">
          {form.parameters.length > 0 && (
            <div className="grid grid-cols-12 gap-2 px-3 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
              <div className="col-span-4 text-left">Param Name</div>
              <div className="col-span-3 text-left">Type</div>
              <div className="col-span-3 text-left">Default Value</div>
          ...
          <div className="space-y-3">
            {form.parameters.length === 0 ? (
              <div className="text-center py-8 border border-dashed rounded-lg bg-slate-50/50">
                <p className="text-xs text-slate-400">No parameters defined yet.</p>
              </div>
            ) : (
              form.parameters.map((p: any, idx: number) => (
                <div 
                  key={idx} 
                  className="grid grid-cols-12 gap-2 items-center bg-muted p-3 rounded-lg border border-slate-200 shadow-sm animate-in slide-in-from-top-1"
                >
                  <div className="col-span-4">
                    <Input 
                      className="h-9 text-xs focus-visible:ring-primary" 
                      placeholder="e.g. timeout_sec" 
                      value={p.paramName} 
                  ...
                  <div className="col-span-3">
                    <Select 
                      value={p.paramType} 
                      onValueChange={(v) => updateParameter(idx, "paramType", v)}
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="SCRIPT_FILE">Script File</SelectItem>
                        <SelectItem value="DEPENDENCY_FILE">Dependency File</SelectItem>
                        <SelectItem value="SCRIPT_ARGUMENT">Script Argument</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="col-span-1 flex flex-col items-center justify-center gap-1">
                    <Switch 
                      checked={p.required} 
                      onCheckedChange={(v) => updateParameter(idx, "required", v)} 
                      className="scale-75"
                    />
                  </div>
            ...
            <div className="flex items-center w-30 gap-3 border px-4 h-10 rounded-md bg-slate-50">
                      <Label className="text-xs font-bold uppercase text-slate-500">Active</Label>
                ...
                <div className="space-y-6 animate-in zoom-in-95">
                  <div className="grid grid-cols-2 gap-6 p-6 border rounded-xl bg-muted">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-semibold">{form.templetName}</p>
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs">{form.targetPlatforms.join(", ") || "None selected"}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Assets</p>
                      <p className="text-xs">{form.dependencyFileIds.length} dependencies uploaded</p>
                    </div>
                  </div>
                  {form.scriptCategory === "TEXT" && (
                    <div className="p-4 bg-slate-900 rounded-lg max-h-40 overflow-auto">
                      <pre className="text-green-500 font-mono text-xs">{form.command}</pre>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="px-8 py-4 bg-muted border-t flex justify-between">
              ...
              <div className="flex gap-2">
                ...
                {step < 3 ? (
                  <Button onClick={() => setStep(step + 1)}  disabled={!isStepValid() || uploadMutation.isPending}>Next Step</Button>
                ) : (
                  <Button className="bg-green-600 hover:bg-green-700 text-white px-8" onClick={handleFinalSubmit}  disabled={!isStepValid() || submitMutation.isPending}>
                    {submitMutation.isPending ? "Saving..." : "Create Template"}
                  </Button>
                )}
              </div>
            </div>
  ...
  <div className="flex flex-col items-center justify-center py-16 animate-in zoom-in-95">
    ...
    <p className="text-sm text-muted-foreground mt-2 text-center max-w-md">
      Your script template <span className="font-semibold">{form.templetName}</span> has been created and is ready to run or manage.
    </p>
