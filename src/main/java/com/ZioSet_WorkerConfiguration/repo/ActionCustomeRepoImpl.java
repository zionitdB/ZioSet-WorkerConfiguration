...
public class ActionCustomeRepoImpl implements ActionCustomeRepo {

	  @PersistenceContext
	  EntityManager entityManager;
	@Override
	public List<Action> getActionsByLimit(int pageNo, int perPage) {
		// TODO Auto-generated method stub
		 try {
			 long result = 0L;
	      Query q = null;
	      result = ((Long)this.entityManager
	        .createQuery("SELECT count(*) FROM Action a ")
	        
	        .getSingleResult()).longValue();
	      TypedQuery typedQuery = this.entityManager.createQuery("select a from  Action a ", Action.class);
	      System.out.println("Count  " + result);
	      int first = (pageNo - 1) * perPage;
	      typedQuery.setFirstResult(first);
	      typedQuery.setMaxResults(perPage);
	      List<Action> list = typedQuery.getResultList();
	      return list;
	    } finally {
	...
	@Override
	public List<Action> getAllActionByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		// TODO Auto-generated method stub
		  int pageNo = groupSearchDTO.getPageNo();
		    int perPage = groupSearchDTO.getPerPage();
		    Query q = null;
		    String queryStr = "from Action a where  ";
		    int i = 0;
		    ...
		    TypedQuery typedQuery = this.entityManager.createQuery(queryStr, Action.class);
		    int j = 0;
		    for (ColumnSearch columnSearch : groupSearchDTO.getColumns()) {
		      if (columnSearch.getValue() != "" || !columnSearch.getValue().equalsIgnoreCase("")) {
	...
	@Override
	public int getCountAllActionByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		// TODO Auto-generated method stub
		  int pageNo = groupSearchDTO.getPageNo();
		    int perPage = groupSearchDTO.getPerPage();
		    Query q = null;
