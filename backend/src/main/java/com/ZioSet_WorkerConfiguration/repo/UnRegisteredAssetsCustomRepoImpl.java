...
public class UnRegisteredAssetsCustomRepoImpl implements UnRegisteredAssetsCustomRepo{
    @PersistenceContext
    EntityManager entityManager;

    @Override
    public List<UnRegisteredAssets> getUnRegisteredAssetsByLimit(int pageNo, int perPage) {
        try {
            long result = 0L;
            Query q = null;
            result = ((Long)this.entityManager
                    .createQuery("SELECT count(*) FROM UnRegisteredAssets a ")

                    .getSingleResult()).longValue();
            TypedQuery typedQuery = this.entityManager.createQuery("select a from  UnRegisteredAssets a ", UnRegisteredAssets.class);
            System.out.println("Count  " + result);
            int first = (pageNo - 1) * perPage;
            typedQuery.setFirstResult(first);
            typedQuery.setMaxResults(perPage);
            List<UnRegisteredAssets> list = typedQuery.getResultList();
            return list;
        } finally {
    ...
    @Override
    public List<UnRegisteredAssets> getAllUnRegisteredAssetsByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
        int pageNo = groupSearchDTO.getPageNo();
        int perPage = groupSearchDTO.getPerPage();
        Query q = null;
        String queryStr = "from UnRegisteredAssets a where  ";
        int i = 0;
        ...
        TypedQuery typedQuery = this.entityManager.createQuery(queryStr, UnRegisteredAssets.class);
        int j = 0;
        for (ColumnSearch columnSearch : groupSearchDTO.getColumns()) {
            if (columnSearch.getValue() != "" || !columnSearch.getValue().equalsIgnoreCase("")) {
    ...
    @Override
    public int getCountAllUnRegisteredAssetsByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
        int pageNo = groupSearchDTO.getPageNo();
        int perPage = groupSearchDTO.getPerPage();
        Query q = null;
        String queryStr = "from UnRegisteredAssets a where  ";
