...
import { Autocomplete, FormControl, IconButton, Input } from "@mui/material";
...
function ConfigurationForm({
  ...
  const getNewCommandId = async () => {
    const url = "configuration/getNewCommandId";
    const { data, error } = await getAgentRequest(url);
    setNewCommandId(data);
    
    
     // Set default value in commandstr field with the fetched NewCommandId
    //  setFormRows([{ commandstr: data.toString(), schemastr: "" }]);
    //  
  };
  ...
  const updateCommand = async () => {
    const request = {
      ...
      list: formRows.map((row) => ({
        ...row,
        commandId: NewCommandId, // Use NewCommandId for each entry in list
      })),
    };
...
//     // Call getNewCommandId when the modal opens
//     getNewCommandId();
    
//     // If in edit mode and selectedData has commandId, set it
...
// }, [isModalOpen]);
  ...
  const handleSubmit = () => {
    if (validation()) {
      if (isEditMode) {
        updateCommand(); // Call update API if in edit mode
      } else {
        addCommand(); // Otherwise, call add API
      }
    } else {
      showToast(errorMessage, ToastTypes.error);
    }
