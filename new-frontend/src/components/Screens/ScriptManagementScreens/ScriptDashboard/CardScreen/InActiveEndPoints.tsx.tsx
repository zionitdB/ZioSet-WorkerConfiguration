...
import {
  useGetInActiveAssets,
  useGetInActiveAssetCount,
  useSearchInActiveAssets,
  useSearchInActiveAssetCount,
} from "./hooks";
import { Input } from "@/components/ui/input";

const InActiveEndPointsRoute = () => {
  ...
  const { data: rowData, isLoading, refetch } = useGetInActiveAssets(page, rowsPerPage, selectedDate);
  const { data: standardTotalCount } = useGetInActiveAssetCount(selectedDate);


  const searchMutation = useSearchInActiveAssets();
  const searchCountMutation = useSearchInActiveAssetCount();
  ...
  useEffect(() => {
   ...
   const { data: getAllData } = useGetInActiveAssets(1, totalItems, selectedDate);
...
const dateFilter = (
  ...
  return (
     ...
     <h1 className="text-3xl font-bold tracking-tight text-foreground">
            <span className="text-primary">In-Active</span> Endpoint
          </h1>
...
export default InActiveEndPointsRoute;
